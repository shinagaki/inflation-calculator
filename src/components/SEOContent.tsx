    ...
    <div className='mt-8 space-y-6 text-sm text-gray-700 bg-white/80 backdrop-blur-sm rounded-lg p-6'>
      <section>
        <h2 className='text-xl font-bold mb-4 text-gray-900'>
          {year}年の{currencyName}の価値を現在の日本円で計算
        </h2>
        
        <div className='space-y-3'>
          <p>
            {year}年の{amount}{currency.toUpperCase()}は、現在の日本円でどのくらいの価値があるのでしょうか？
        ...
        <div className='space-y-4'>
          <div>
            ...
            <div className='flex flex-wrap gap-2'>
              {[
                { year: '1950', era: '戦後復興期' },
                { year: '1964', era: '東京オリンピック' },
                { year: '1973', era: 'オイルショック' },
                { year: '1980', era: '高度成長期' },
                { year: '1990', era: 'バブル期' },
                { year: '2000', era: 'IT革命' },
                { year: '2008', era: 'リーマンショック' },
                { year: '2020', era: 'コロナ時代' }
              ].map(item => (
                <div key={item.year} className='bg-blue-100 px-3 py-1 rounded-full'>
                  <span className='text-xs font-medium text-blue-800'>
                    {item.year}年（{item.era}）
                  </span>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs'>
              <div className='bg-green-50 p-3 rounded border border-green-200'>
                ...
                <p>「1980年に100万円投資していたら現在価値でいくら？」</p>
              </div>
              <div className='bg-purple-50 p-3 rounded border border-purple-200'>
                <strong>歴史・教育</strong>
                <p>「戦後の初任給1万円は現在のいくらに相当？」</p>
              </div>
              <div className='bg-orange-50 p-3 rounded border border-orange-200'>
                <strong>経済分析</strong>
                <p>「バブル期の土地価格を現在価値で比較」</p>
              </div>
              <div className='bg-cyan-50 p-3 rounded border border-cyan-200'>
                <strong>国際比較</strong>
                <p>「1990年の1ドルの購買力は現在いくら？」</p>
              </div>
            </div>
          </div>
        </div>
