...
describe('InflationResult', () => {
  describe('ローディング状態', () => {
    it('loading=trueの時にローディングメッセージが表示される', () => {
      ...
      expect(
        screen.queryByText(defaultProps.resultStatement),
  ...
  describe('未計算状態', () => {
    it('result=undefinedの時に初期メッセージが表示される', () => {
      ...
      expect(
        screen.getByText('西暦を選択し、金額に数値を入れて、通貨を選択します'),
      ).toBeInTheDocument()
  ...
  describe('正常な結果表示', () => {
    ...
    it('SNS共有ボタンに正しいpropsが渡される', async () => {
      ...
      expect(twitterButton).toHaveAttribute(
        'data-url',
        'https://imaikura.creco.net/1980/usd/100',
      )
      expect(twitterButton).toHaveAttribute(
        'data-title',
        defaultProps.shareStatement,
      )

      expect(lineButton).toHaveAttribute(
        'data-url',
        'https://imaikura.creco.net/1980/usd/100',
      )
      expect(lineButton).toHaveAttribute(
  ...
  describe('スタイリング', () => {
    it('結果表示部分に適切なクラスが適用される', () => {
      ...
      expect(resultElement).toHaveClass(
        'text-zinc-900',
